<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body>
	<button type="button">메인요리</button>
	<br>
	<button type="button">초대요리</button>
	<br>
	<button type="button">간단요리</button>
	<br>
	<button type="button">밑 반찬</button>
	<br>
	<button type="button">간식</button>
	<br>
	<button type="button">안주</button>
	<br>
	<button type="button">베이킹</button>
	<br>
	<button type="button">다이어트</button>
	<br>
	<button type="button">도시락</button>
	<br>
	<button type="button">한식</button>
	<br>
	<button type="button">양식</button>
	<br>
	<button type="button">일식</button>
	<br>
	<button type="button">중식</button>
	<br>
	<button type="button">채식</button>
	<br>
	<button type="button">퓨전</button>
	<br>
	<button type="button" onclick="location.href='Main.jsp?sidePage=../Food/foodside.jsp&contentPage=../Food/foodbody.jsp">모두 보기</button>

</body>
</html>